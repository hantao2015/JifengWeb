...
{
    public partial class ShopSubmit : System.Web.UI.Page
    {
        ...
        protected void btn_update_Click(object sender, EventArgs e)
        {
            ...
            int num3;
            int num4;
            int count = this.Repeater_Product.Items.Count;
            for (num2 = 0; num2 < count; num2++)
            {
                box = this.Repeater_Product.Items[num2].FindControl("txt_num") as TextBox;
                button = this.Repeater_Product.Items[num2].FindControl("Ibtn_Delete") as ImageButton;
                if ((box != null) && (button != null))
                {
                    ...
                    {
                        base.ClientScript.RegisterStartupScript(base.GetType(), "", "<script>alert('数量只能是正整数!');</script>");
                        return;
                    }
                    if (!int.TryParse(button.CommandArgument, out num4))
                    {
                        base.ClientScript.RegisterStartupScript(base.GetType(), "", "<script>alert('请重试!');</script>");
                        return;
                    }
                }
                else
                {
                    base.ClientScript.RegisterStartupScript(base.GetType(), "", "<script>alert('数量有误，请重新输入!');</script>");
                    return;
                }
            }
            ...
            for (num2 = 0; num2 < count; num2++)
            {
                box = this.Repeater_Product.Items[num2].FindControl("txt_num") as TextBox;
                button = this.Repeater_Product.Items[num2].FindControl("Ibtn_Delete") as ImageButton;
                num3 = int.Parse(box.Text.Trim());
                num4 = int.Parse(button.CommandArgument);
                ShopCartModel model = tbll.GetModel(num4);
                model.Integral = (model.Integral / model.ProductNum) * num3;
                model.ProductNum = num3;
                if (!tbll.EidtRow(model))
                {
                    flag = false;
                    break;
                }
            ...
            if (!flag)
            {
                base.ClientScript.RegisterStartupScript(base.GetType(), "", "<script>alert('更新有误，请删除后添加！');</script>");
            }
            else
            {
                this.Master.UpdateLoginUser();
                this.BandProduct();
                base.ClientScript.RegisterStartupScript(base.GetType(), "", "<script>alert('更新数量成功！');</script>");
            }
        }
