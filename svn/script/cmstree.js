...
DFlowTree.prototype.nodeInit = function(sourceIndex, parentId)
{
  ...
  var num=this.getAttribute(source, "num");
...
DFlowTree.prototype.nodeToHTML = function(node, AtEnd)
{
  ...
  if(target=="Popup")
  {
      ...
      HTML += "<IMG "+
       ...
       // "href='"+ "#" +"' "+
        ...
        "onclick=\"return "+ this.name +".nodeClickPopup('"+ url +"')\">"+ node.text +
      "</A></NOBR></DIV>";
  ...
  else
  {
      ...
      "<span style='color:red'>"+node.num+"</span></A></NOBR></DIV>";
  }
...
DFlowTree.prototype.nodeClickPopup = function(url)
{
    
     
    window.open(url, 'newwindow','alwaysRaised=yes,depended=yes,height=800,width=1200,top=0,left=200,toolbar=no,menubar=no,scrollbars=yes, resizable=yes,location=no, status=no,titlebar=no');
  //  var source = this.nodes[this.node[id].sourceIndex];
    //eval(this.getAttribute(source, "method"));
    return true;
};
...
function DFlowTree(strTreeName)
{
	...
	this._d    = "\x0f";
	this.index = 0;
	this.divider   = "~"; //"_"; 节点ID之间的连接符号，需要改进连接符号，只需在这里改变即可，但必须是一个字符。
	this.rootId = "-1";
